...
## 西8 — JSON 鲁棒/返回前进/语言TTS/补全分组控制（8项）（2026-07-11）

- **AI 新词汇跳转修复**（#1）：添加后清空筛选 + 自动创建缺失的词根条目 + 选中高亮新词条并滚动到位，不再跳转到空分组。
- **JSON 提取鲁棒**（#1-1）：`extractJSON` 新增尾随逗号移除 + 单行/块注释清理 + 多层代码块处理，修复「响应未找到 JSON」和「Expected ']'」错误。
- **全局导航前进**（#2）：新增 `navForward()` + `→` 前进按钮；`←` `→` 均支持 Alt+← / Alt+→ 快捷键；历史栈上限从 60→80。**注意：页面刷新后历史清空**（内存级别）。
- **语音 TTS 语言对应**（#4）：`playButton(w, size, lang)` 接受中文语言名而非 BCP-47 代码，避免双重转码导致所有外语都读成西语。英语→en-US、意→it-IT、法→fr-FR 等一一对应。
- **标签浏览器 PCIC 标记**（#5-1）：`renderTagTree` 中官方 20 大类新增 🔷 标记，与标签云风格统一。
- **标签层级分隔改善**（#5-2）：一级▸二级的箭头改为 `text-white/50 ▸`（更亮）+ 不同语义标签对之间加 `·` 分隔点。
- **AI 补全分区控制**（#6-2）：每个字段分组（内容/分类/词法）新增独立「仅空」切换，`completionConfig.groupOnlyIfEmpty` 按组存储。`_isCompletionNeeded` 按组判断。
- **形态拆解注释尺寸**（#7）：语素旁的解释文字从 `text-[10px] text-slate-500` 加大到 `text-[11px] text-slate-300`。
